...
public class WebSocketMessageConfirmationServiceImpl implements WebSocketMessageConfirmationService {
    ...
    public String sendConfirmableMessage(String username, WebSocketMessageDTO message) {
        try {
            ...
            log.info("已发送需要确认的消息: username={}, messageId={}", username, message.getMessageId());
            return message.getMessageId();
        } catch (Exception e) {
            log.error("发送需要确认的消息失败: username={}, messageId={}", username, message.getMessageId(), e);
        ...
        try {
            if (ack.getAcknowledged() != null && ack.getAcknowledged()) {
                ...
                if (message != null) {
                    ...
                    log.warn("未找到需要确认的消息: messageId={}", ack.getMessageId());
                }
            }
        } catch (Exception e) {
